...
export async function getCategoryPath(subcategoryId: string): Promise<string[]> {
  if (!subcategoryId) {
  ...
  if (category.parent) {
    ...
    if (category.parent.parent) {
      ...
      mainCategory = category.parent.parent.name.toLowerCase().replace(/\s+/g, '-');
    }
  }


  if (!mainCategory || !subCategory || !subSubCategory) {
    throw new Error(`Incomplete category hierarchy for subcategory_id: ${subcategoryId}`);
  }

...
export default function Step1({ onContinue, formData }: Step1Props) {
  ...
  // Add state to track if the form is valid
  const isFormValid = useMemo(() => {
    const plainTextDescription = description.replace(/(<([^>]+)>)/gi, "");
    return (
      title.trim().length >= 3 &&
      title.trim().length <= 100 &&
      plainTextDescription.length >= 30 &&
      plainTextDescription.length <= 1500
    );
  }, [title, description]);
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p
                  className={`text-sm mt-1 ${
                    description.length > 1500 ? "text-red-500" : "text-gray-500"
                  }`}
                >
